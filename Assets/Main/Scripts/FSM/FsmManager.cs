...
namespace BigHead.Fsm
{
    ...
    /// 有限状态机管理器。
    /// </summary>
    internal sealed class FsmManager : BigHeadModule, IFsmManager
    {
        ...
        public bool HasFsm(Type ownerType)
        {
            if (ownerType == null)
            {
                throw new BigHeadException("Owner type is invalid.");
            }
        ...
        public IFsm<T> CreateFsm<T>(string name, T owner, params FsmState<T>[] states) where T : class
        {
            if (HasFsm<T>(name))
            {
                throw new BigHeadException(string.Format("Already exist FSM '{0}'.", Utility.Text.GetFullName<T>(name)));
            }
        ...
        public bool DestroyFsm<T>(IFsm<T> fsm) where T : class
        {
            if (fsm == null)
            {
                throw new BigHeadException("FSM is invalid.");
            }
