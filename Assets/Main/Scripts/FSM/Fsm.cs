...
namespace BigHead.Fsm
{
    ...
    internal sealed class Fsm<T> : FsmBase, IFsm<T> where T : class
    {
            ...
            : base(name)
        {
            if (owner == null)
            {
                throw new BigHeadException("FSM owner is invalid.");
            }

            if (states == null || states.Length < 1)
            {
                throw new BigHeadException("FSM states is invalid.");
            }
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new BigHeadException(string.Format("FSM '{0}' state '{1}' is already exist.", Utility.Text.GetFullName<T>(name), stateName));
                }
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            if (IsRunning)
            {
                throw new BigHeadException("FSM is running, can not start again.");
            }

            FsmState<T> state = GetState<TState>();
            if (state == null)
            {
                throw new BigHeadException(string.Format("FSM '{0}' can not start state '{1}' which is not exist.", Utility.Text.GetFullName<T>(Name), typeof(TState).FullName));
            }
        ...
        public void Start(Type stateType)
        {
            ...
            if (stateType == null)
            {
                throw new BigHeadException("State type is invalid.");
            }

            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
            {
                throw new BigHeadException(string.Format("State type '{0}' is invalid.", stateType.FullName));
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new BigHeadException(string.Format("FSM '{0}' can not start state '{1}' which is not exist.", Utility.Text.GetFullName<T>(Name), stateType.FullName));
            }
        ...
        public void FireEvent(object sender, int eventId)
        {
            ...
            {
                throw new BigHeadException("Current state is invalid.");
            }
        ...
        public bool HasData(string name)
        {
            ...
            {
                throw new BigHeadException("Data name is invalid.");
            }
        ...
        internal void ChangeState(Type stateType)
        {
            ...
            if (state == null)
            {
                throw new BigHeadException(string.Format("FSM '{0}' can not change state to '{1}' which is not exist.", Utility.Text.GetFullName<T>(Name), stateType.FullName));
            }
